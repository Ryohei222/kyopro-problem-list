...
export function UserProblemLists({
  lists,
  emptyMessage,
  showAuthor,
}: UserProblemListsProps) {
        ...
        <Card key={list.id} className="hover:shadow-md transition-shadow">
          <CardContent className="pt-6">
            <Link href={`/list/${list.id}`} className="block">
              <h3 className="text-lg font-bold hover:text-blue-600 transition-colors line-clamp-2">
                {list.name}
              </h3>
            </Link>
